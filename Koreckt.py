...
# Данные для авторизации (временное хранение)
auth_data = {
    "phone": None,
    "code": None,
    "password": None,
    "step": None,  # 'phone', 'code', 'password'
    "client": None
}
...
def pro_sender_engine():
    ...
    # Проверяем, есть ли уже авторизованный клиент
    if client is None:
...
def login_cmd(msg):
    """Начало процесса авторизации в аккаунте Telegram"""
...
def process_auth_step(message):
    ...
    if auth_data["step"] == "phone":
        ...
        try:
            ...
            # Авторизация успешна, сохраняем клиент
            global client
            client = auth_data["client"]
            
            # Перезапускаем движок с новым клиентом
...
def restart_engine_with_client():
    """Перезапуск движка рассылки с новым клиентом"""
    global mailing_thread, CONFIG
    
    # Если движок был запущен, перезапускаем его
    was_running = CONFIG["mailing_enabled"]
    
    if was_running:
        # Временно останавливаем рассылку
        CONFIG["mailing_enabled"] = False
        time.sleep(2)
    
    # Перезапускаем поток
    if mailing_thread and mailing_thread.is_alive():
        # Старый поток продолжит работу, но увидит что client изменился
        pass
    else:
...
# Обработчик для шагов авторизации (должен быть после всех команд)
